...
import { t as require_react_dom } from "./react-dom-Cug5xZdF.js";
...
function createMemoryHistory(options = {}) {
	...
	function createMemoryLocation(to, state = null, key, mask) {
		let location$1 = createLocation(entries ? getCurrentLocation().pathname : "/", to, state, key, mask);
...
function getUrlBasedHistory(getLocation, createHref2, validateLocation, options = {}) {
	...
	function push(to, state) {
		...
		let url = history.createHref(location$1.mask || location$1);
		try {
...
function createBrowserURLImpl(windowImpl, to, isAbsolute = false) {
	...
	if (!isAbsolute && PROTOCOL_RELATIVE_URL_REGEX.test(href$1)) href$1 = base + href$1;
...
function createRouter(init) {
	...
	async function startNavigation(historyAction, location$1, opts) {
		...
		let matches = opts?.initialHydration && state.matches && state.matches.length > 0 && !initialMatchesIsFOW ? state.matches : matchRoutesImpl(routesToUse, location$1, basename, false, dataRoutes.branches);
		let flushSync = (opts && opts.flushSync) === true;
		...
		if (opts && opts.pendingError) pendingActionResult = [findNearestBoundary(matches).route.id, {
			...
			let actionResult = await handleAction(request, location$1, opts.submission, matches, historyAction, scopedContext, fogOfWar.active, opts && opts.initialHydration === true, {
			...
			loadingNavigation = getLoadingNavigation(location$1, matches, historyAction, opts.submission);
			flushSync = false;
			fogOfWar.active = false;
			request = createClientSideRequest(init.history, request.url, request.signal);
		}
		let { shortCircuited, matches: updatedMatches, loaderData, errors, workingFetchers } = await handleLoaders(request, location$1, matches, historyAction, scopedContext, fogOfWar.active, loadingNavigation, opts && opts.submission, opts && opts.fetcherSubmission, opts && opts.replace, opts && opts.initialHydration === true, flushSync, pendingActionResult, opts && opts.callSiteDefaultShouldRevalidate);
	...
	async function handleAction(request, location$1, submission, matches, historyAction, scopedContext, isFogOfWar, initialHydration, opts = {}) {
		interruptActiveLoads();
		updateState({ navigation: getSubmittingNavigation(location$1, matches, historyAction, submission) }, { flushSync: opts.flushSync === true });
		if (isFogOfWar) {
	...
	async function handleLoaders(request, location$1, matches, historyAction, scopedContext, isFogOfWar, overrideNavigation, submission, fetcherSubmission, replace2, initialHydration, flushSync, pendingActionResult, callSiteDefaultShouldRevalidate) {
		let loadingNavigation = overrideNavigation || getLoadingNavigation(location$1, matches, historyAction, submission);
	...
	async function startRedirectNavigation(request, redirect2, isNavigation, { submission, fetcherSubmission, preventScrollReset, replace: replace2 } = {}) {
		...
		if (isBrowser3) {
			...
			if (redirect2.response.headers.has("X-Remix-Reload-Document")) isDocumentReload = true;
				...
				const url = createBrowserURLImpl(routerWindow, location$1, true);
	...
	if (init.instrumentations) router$1 = instrumentClientSideRouter(router$1, init.instrumentations.map((i) => i.router).filter(Boolean));
	return router$1;
}
function createStaticHandler(routes, opts) {
	...
	async function query(request, { requestContext, filterMatchesToLoad, skipLoaderErrorBubbling, skipRevalidation, dataStrategy, generateMiddlewareResponse, normalizePath } = {}) {
		...
		let location$1 = createLocation("", normalizePathImpl(request), null, "default");
		let matches = matchRoutesImpl(dataRoutes, location$1, basename, false, routeBranches);
		...
		if (generateMiddlewareResponse) {
			...
			try {
				...
				let response = await runServerMiddlewarePipeline({
					request,
					url: createDataFunctionUrl(request, location$1),
...
function hasInvalidProtocol(location$1) {
	try {
		return invalidProtocols.includes(new URL(location$1).protocol);
	} catch {
		return false;
	}
}
function normalizeRedirectLocation(location$1, currentUrl, basename, historyInstance) {
	...
	try {
		if (hasInvalidProtocol(historyInstance.createURL(location$1).toString())) throw new Error("Invalid redirect location");
	} catch (e) {}
	return location$1;
}
...
function getLoadingNavigation(location$1, matches, historyAction, submission) {
...
function getSubmittingNavigation(location$1, matches, historyAction, submission) {
	return {
		state: "submitting",
		location: location$1,
		matches,
...
function useRouterState() {
	let { location: location$1, historyAction: type, matches, navigation } = useDataRouterState("unstable_useRouterState");
	let active = import_react.useMemo(() => ({
		type,
		location: location$1,
		searchParams: new URLSearchParams(location$1.search),
...
function RouterProvider({ router: router$1, flushSync: reactDomFlushSyncImpl, onError, useTransitions }) {
...
function encode(input, options) {
	...
	return new ReadableStream({ async start(controller) {
		const id = await flatten.call(encoder$1, input);
				...
				for (const [deferredId, deferred] of Object.entries(encoder$1.deferred)) {
					...
					seenPromises.add(encoder$1.deferred[Number(deferredId)] = Promise.race([racePromise, deferred]).then((resolved) => {
						processingChain = processingChain.then(async () => {
							const id2 = await flatten.call(encoder$1, resolved);
...
`));
						...
						processingChain = processingChain.then(async () => {
							...
							const id2 = await flatten.call(encoder$1, reason);
							if (Array.isArray(id2)) {
...
function decodeViaTurboStream(body, global$1) {
	return decode$1(body, { plugins: [(type, ...rest) => {
		if (type === "SanitizedError") {
			...
			if (name && SUPPORTED_ERROR_TYPES.includes(name) && name in global$1 && typeof global$1[name] === "function") Constructor = global$1[name];
...
function RemixRootDefaultHydrateFallback() {
	...
	return /* @__PURE__ */ import_react.createElement(BoundaryShell, {
		title: "Loading...",
		renderScripts: true
	}, ENABLE_DEV_WARNINGS ? /* @__PURE__ */ import_react.createElement("script", {
		nonce,
...
function getPartialManifest({ sri, ...manifest }, router$1) {
	...
	paths.forEach((path) => {
		let matches = matchRoutesImpl(router$1.routes, path, router$1.basename || "/", false, router$1.branches);
...
var Link = import_react.forwardRef(function LinkWithRef({ onClick, discover = "render", prefetch = "none", relative, reloadDocument, replace: replace2, mask, state, target, to, preventScrollReset, viewTransition, defaultShouldRevalidate, ...rest }, forwardedRef) {
	...
	let maskedHref = null;
	if (mask) {
		let resolved = resolveTo(mask, [], location$1.mask ? location$1.mask.pathname : "/", true);
...
function encodeLocation(to) {
	let href$1 = typeof to === "string" ? to : createPath(to);
	href$1 = href$1.replace(/ $/, "%20");
	let encoded = ABSOLUTE_URL_REGEX.test(href$1) ? new URL(href$1) : new URL(href$1, "http://localhost");
	return {
...
function ServerRouter({ context, url, nonce }) {
	...
	let router$1 = createStaticRouter(routes, context.staticHandlerContext, { branches: context.branches });
...
async function callRouteHandler(handler, args, future) {
	let result = await handler({
		request: future.v8_passThroughRequests ? args.request : stripRoutesParam(stripIndexParam$1(args.request)),
		url: args.url,
		params: args.params,
...
async function handleManifestRequest(build, dataRoutes, branches, url) {
	if (url.toString().length > URL_LIMIT) return new Response(null, {
		statusText: "Bad Request",
		status: 400
	});
