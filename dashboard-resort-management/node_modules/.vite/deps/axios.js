...
var isPlainObject = (val) => {
	...
	return (prototype$1 === null || prototype$1 === Object.prototype || getPrototypeOf(prototype$1) === null) && !hasOwnInPrototypeChain(val, toStringTag) && !hasOwnInPrototypeChain(val, iterator);
};
/**
...
function throwIfDepthExceeded(index) {
	if (index > MAX_DEPTH) throw new AxiosError_default("FormData field is too deeply nested (" + index + " levels). Max depth: " + MAX_DEPTH, AxiosError_default.ERR_FORM_DATA_DEPTH_EXCEEDED);
}
/**
...
function assertValidHttpProtocolURL(url, config) {
	if (typeof url === "string" && malformedHttpProtocol.test(normalizeURLForProtocolCheck(url))) throw new AxiosError_default("Invalid URL: missing \"//\" after protocol", AxiosError_default.ERR_INVALID_URL, config);
}
/**
...
function resolveConfig(config) {
	...
	newConfig.url = buildURL(buildFullPath(baseURL, url, allowAbsoluteUrls, newConfig), own$1("params"), own$1("paramsSerializer"));
	if (auth) {
...
var composeSignals = (signals, timeout) => {
	...
	signals.forEach((signal$1) => signal$1.addEventListener("abort", onabort, { once: true }));
...
var factory = (env) => {
	...
	return async (config) => {
		...
		const own$1 = (key) => utils_default.hasOwnProp(config, key) ? config[key] : void 0;
		...
		try {
			...
			else if (mustEnforceStreamBody && isRequestSupported && !supportsRequestStream && method !== "get" && method !== "head") throw new AxiosError_default("Stream request bodies are not supported by the current fetch implementation", AxiosError_default.ERR_NOT_SUPPORT, config, request);
...
function assertOptions(options, schema, allowUnknown) {
	if (typeof options !== "object" || options === null) throw new AxiosError_default("options must be an object", AxiosError_default.ERR_BAD_OPTION_VALUE);
...
//#endregion
export { Axios, AxiosError, AxiosHeaders, Cancel, CancelToken, CanceledError, HttpStatusCode, VERSION, all, create, axios_default as default, formToJSON, getAdapter, isAxiosError, isCancel, mergeConfig, spread, toFormData };
