...
function DefaultBadge() {
  return (
    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-semibold bg-[#F5F8FC] text-[#829AB1] border border-[#D9E2EC]">
      Default
    </span>
  );
}
...
function VideoUpload({ section, fieldKey, label, currentUrl, currentId, onSaved }) {
  ...
  const borderColor = file ? "border-[#FF6B00]" : isLive ? "border-[#86EFAC]" : "border-[#D9E2EC]";

  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-center gap-2">
        <span className="text-[14px] font-semibold text-[#486581]">{label}</span>
        {isLive && <LiveBadge />}
        {!currentUrl && !file && <DefaultBadge />}
        <span className="ml-auto text-[12px] text-[#829AB1]">Max {VIDEO_MAX_MB} MB &middot; mp4 / webm / mov</span>
      </div>

      <div className={`relative rounded-[10px] overflow-hidden border-2 ${borderColor} bg-[#F5F8FC] transition-colors`} style={{ height: 200 }}>
        {preview ? (
          <>
          ...
          <div className="w-full h-full flex flex-col items-center justify-center gap-2">
            <FaImage size={32} className="text-[#D9E2EC]" />
            <span className="text-[13px] text-[#486581]">No video uploaded — default video will play</span>
            <span className="text-[12px] text-[#829AB1]">Accepted: mp4, webm, mov &middot; Max {VIDEO_MAX_MB} MB</span>
          </div>
        )}
      </div>
      ...
      {saving && (
        <div className="w-full">
          <div className="flex justify-between text-[12px] text-[#486581] mb-1">
            <span>Uploading…</span>
            <span>{progress}%</span>
          </div>
          <div className="w-full h-1.5 bg-[#D9E2EC] rounded-full overflow-hidden">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <label htmlFor={id} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-[8px] text-[13px] font-semibold cursor-pointer border border-[#D9E2EC] bg-white text-[#486581] hover:bg-[#F5F8FC] transition-colors ${
        ...
        {isLive && !file && (
          <button onClick={handleDelete} disabled={deleting || saving}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-[8px] text-[13px] font-semibold bg-red-50 text-red-600 border border-red-200 hover:bg-red-100 disabled:opacity-60 transition-colors">
          ...
          <>
            ...
            {!saving && (
              <button onClick={() => { setFile(null); setProgress(0); }}
                className="p-1.5 rounded-[8px] text-[#829AB1] hover:text-[#102A43] hover:bg-[#F5F8FC] transition-colors">
                <MdClose size={14} />
              </button>
            )}
            <span className="text-[12px] text-[#486581]">
...
function ImageUpload({ section, fieldKey, label, currentUrl, currentId, defaultImage, onSaved }) {
  ...
  const borderColor = file
    ? "border-[#FF6B00]"
    : isLive ? "border-[#86EFAC]" : "border-[#D9E2EC]";
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className={`relative rounded-[10px] overflow-hidden border-2 ${borderColor} bg-[#F5F8FC] transition-colors`} style={{ height: 160 }}>
        {preview ? (
          <>
          ...
          <div className="w-full h-full flex flex-col items-center justify-center gap-2">
            <FaImage size={32} className="text-[#D9E2EC]" />
            <span className="text-[13px] text-[#486581]">No image set</span>
          </div>
        )}
      </div>
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {file && (
          <>
            ...
            <span className="text-[12px] truncate max-w-[120px] text-[#486581]">{file.name}</span>
          </>
        )}
      </div>
    </div>
...
function TextInput({ section, fieldKey, label, type, defaultValue, currentValue, onSaved }) {
  ...
  const borderColor = dirty ? "border-[#FF6B00]" : isLive ? "border-[#86EFAC]" : "border-[#D9E2EC]";
  const inputCls = `w-full border ${borderColor} rounded-[10px] px-5 py-3 text-[17px] text-[#102A43] bg-white focus:outline-none focus:border-[#FF6B00] focus:ring-2 focus:ring-[#FF6B00]/20 transition-colors placeholder:text-[#829AB1] font-[Inter,Poppins,sans-serif]`;

  return (
    <div className="flex flex-col gap-2">
      <div className="flex items-center gap-2">
        <label className="text-[14px] font-semibold text-[#486581]">{label}</label>
        {isLive && <LiveBadge />}
        {isDefault && <DefaultBadge />}
      </div>
      {type === "textarea"
...
function CardGroup({ group, section, allContent, onSaved }) {
  ...
  return (
    <div className="bg-white border border-[#D9E2EC] rounded-xl p-6">
      <div className="flex items-center gap-2 mb-5 pb-4 border-b border-[#D9E2EC]">
        <span className="text-[15px] font-bold text-[#102A43]">{group.groupLabel}</span>
        {imageFields.length > 0 && cardName && (
          <span className="text-[12px] font-semibold px-2.5 py-0.5 rounded-full bg-[#FFF3E8] text-[#FF6B00] border border-[#FFD4A8]">
            {cardName}
          </span>
        )}
        <span className="flex items-center gap-1 text-[13px] text-[#829AB1] ml-1">
...
export default function Manage_WebContent() {
  ...
  return (
    <div className="min-h-full bg-[#F5F8FC] rounded-xl p-6" style={{ fontFamily: "Inter, Poppins, sans-serif" }}>

      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-[26px] font-bold text-[#102A43] flex items-center gap-2.5">
            <MdLanguage className="text-[#FF6B00]" size={26} />
            Website Content Manager
          </h2>
          <p className="text-[15px] text-[#486581] mt-1">
      ...
      <div className="bg-white border border-[#D9E2EC] rounded-xl shadow-sm overflow-hidden">

        <div className="flex overflow-x-auto border-b border-[#D9E2EC] bg-[#F5F8FC]">
          {SECTIONS.map(s => (
