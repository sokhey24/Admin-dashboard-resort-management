...
export function ActionDropdown({ payment, dark, onView }) {
...
export default function PaymentDetailModal({ payment, open, onClose, dark, autoPrint = false }) {
  ...
  useEffect(() => {
    ...
    // If payment object is passed directly (mock data), use it
    if (payment) {
      setData(payment);
    }
  }, [open, payment]);
  ...
  const handlePrint = () => {
    // Log print action (mock)
    console.log('Print logged for payment:', data?.payment_id);
    setShowReceipt(true);
  ...
  const handlePdf = async () => {
    ...
    setTimeout(() => {
      ...
      message.info({ content: `PDF would download: ${data?.payment_id}_Payment_Receipt.pdf`, key: "pdf_modal", duration: 4 });
    }, 1000);
  };
          ...
          <div className="px-5 py-4 max-h-[76vh] overflow-y-auto">
            {loading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div>
                  <Section
                    title={`Source: ${p.source === "resort" ? "Resort Booking" : "Restaurant Order"}`}
                    dark={dark}
                  >
                    {p.source === "resort" ? (
                      <>
                        ...
                        <InfoRow label="Room / Villa"   value={p.reference?.rooms?.map((r) => r.room_number).join(", ")}                                                   dark={dark} />
