...
import { MdSearch, MdCheck, MdClose, MdDelete, MdLogout, MdPerson, MdHotel, MdPayment, MdReceipt } from "react-icons/md";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4 py-6 overflow-y-auto ">
      <div className={`rounded-xl shadow-2xl w-full max-w-2xl ${dark ? "bg-gray-800 border border-gray-700" : "bg-white"}`}>
        <div className={`flex items-center justify-between px-6 py-4 border-b ${dark ? "border-gray-700" : "border-[#D9E2EC]"}`}>
          <div>
            <h3 className={`text-base font-semibold mt-20 ${dark ? "text-gray-100" : "text-[#102A43]"}`}>Check-out Summary</h3>
...
export default function PendingBooking() {
  ...
  const setCheckoutTime = useBookingStore(s => s.setCheckoutTime);
  ...
  const handleCheckoutConfirm = async () => {
    ...
    const res = await request(`admin/bookings/${checkoutDetail.id}`, "put", { status: "completed" });
    setCheckoutLoading(false);
    if (!res?.errors) { message.success("Checked out successfully"); setCheckoutDetail(null); load(); }
  ...
  const handleConfirm = async () => {
    ...
    if (type === "delete") {
      ...
      if (!res?.errors) message.success(`Booking ${status}`);
    }
    if (res?.errors) message.error("Failed");
    setConfirm(null);
    load();
  };
  ...
  const confirmConfig = {
    ...
    approve: { title: "Approve Booking", confirmText: "Yes, Approve", danger: false, sub: null },
    reject:  { title: "Reject Booking",  confirmText: "Yes, Reject",  danger: true,  sub: null },
  };
                ...
                <tr key={b.id} className={`transition-colors ${rowHover}`}>
                  ...
                  <td className="px-4 py-4">
                    <div className="flex items-center justify-center gap-1.5 flex-wrap">
                      <Button onClick={() => setConfirm({ booking: b, type: "approve", loading: false })}
                      ...
                      <Button onClick={() => setConfirm({ booking: b, type: "reject", loading: false })}
                      ...
                      <Button onClick={() => { setCheckoutTime(new Date().toISOString()); setCheckoutDetail(b); }}
                        className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-medium transition-colors ${dark ? "bg-purple-900/40 text-purple-400 hover:bg-purple-900/70" : "bg-purple-50 text-purple-600 hover:bg-purple-100"}`}>
      ...
      {confirm && (() => {
        ...
        return (
          <ConfirmDialog
            ...
            dark={dark}
            title={cfg.title}
            message={`Are you sure you want to ${confirm.type} booking ${code} for ${name}?`}
            sub={cfg.sub}
